...
SIPml.Stack = function(o_conf) {
    ...
    */
    ...
    this.o_stack.on_event_stack = function(e) {
        ...
        switch (e.i_code) {
            ...
            case tsip_event_code_e.STACK_FAILED_TO_START:
                s_type = 'failed_to_start';
                break;
            case tsip_event_code_e.STACK_FAILED_TO_STOP:
                s_type = 'failed_to_stop';
                break;
        }
        if (s_type) {
            e.o_stack.oStack.dispatchEvent({
                s_type: s_type,
                o_value: new SIPml.Stack.Event(s_type, e)
            });
        }
    }
    ...
    this.o_stack.on_event_dialog = function(e) {
        ...
        if (s_type) {
            oSession.dispatchEvent({
                s_type: s_type,
                o_value: new SIPml.Session.Event(oSession, s_type, e)
            });
        }
    }
    ...
    this.o_stack.on_event_message = function(e) {
        ...
        if (s_type) {
            ...
            if (s_type == 'i_new_message') {
                ...
                e.o_session.o_stack.oStack.dispatchEvent({
                    s_type: s_type,
                    o_value: oNewEvent
                });
            } else {
    ...
    this.o_stack.on_event_invite = function(e) {
                    ...
                    default:
                        {
                            oSession.dispatchEvent({
                                s_type: s_event_type,
                                o_value: new SIPml.Session.Event(oSession, s_event_type, e)
                            });
                            break;
                        }
                }
        ...
        switch (e.e_invite_type) {
            ...
            case tsip_event_invite_type_e.I_ECT_NEW_CALL:
                s_type = 'i_ect_new_call';
                break;
            case tsip_event_invite_type_e.I_AO_REQUEST:
                s_type = 'i_ao_request';
                break;
            case tsip_event_invite_type_e.M_EARLY_MEDIA:
                s_type = 'm_early_media';
                break;
            case tsip_event_invite_type_e.M_STREAM_LOCAL_REQUESTED:
                s_type = 'm_permission_requested';
                break;
            case tsip_event_invite_type_e.M_STREAM_LOCAL_ACCEPTED:
                s_type = 'm_permission_accepted';
                break;
            case tsip_event_invite_type_e.M_STREAM_LOCAL_REFUSED:
                s_type = 'm_permission_refused';
                break;
            case tsip_event_invite_type_e.M_STREAM_LOCAL_ADDED:
                {
...
SIPml.Stack.prototype.setConfiguration = function(o_conf) {
    ...
    var o_bandwidth = o_conf.bandwidth ? o_conf.bandwidth : {
        audio: undefined,
        video: undefined
    };
...
SIPml.Stack.prototype.newSession = function(s_type, o_conf) {
    ...
    if (s_type == 'register') {
        ...
        if (oStack.ao_headers && oStack.ao_headers[2] && oStack.ao_headers[2].s_name) {
            oStack.ao_headers[2].s_value = parentWindow.href;
        }
...
SIPml.Session.prototype.setConfiguration = function(o_conf) {
    ...
    if (this instanceof SIPml.Session.Call) {
        ...
        o_session.set(
            tsip_session.prototype.SetBandwidth(o_conf.bandwidth ? o_conf.bandwidth : {
                audio: undefined,
                video: undefined
            }),
